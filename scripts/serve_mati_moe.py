...
class MoEHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/v1/chat/completions":
            ...
            dominant = routing["dominant_expert"].upper()
            weights = routing["weights"]

            response_text = (
                f"[Mati 12B MoE Routed -> {dominant} ({weights[routing['dominant_expert']]*100:.1f}%)]\n"
                f"Processed prompt through MultiLoRA-MoE layers 8..47.\n"
            ...
            response_payload = {
                ...
                "usage": {"prompt_tokens": len(last_prompt.split()), "completion_tokens": 20, "total_tokens": 0},
